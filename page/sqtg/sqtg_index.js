...
Page({
    ...
    GetFormId: function (e) {
      ...
      server.getJSON('https://xcx.so50.com/Pages/ajaxsqtg/GetFormId.ashx', { uid: wx.getStorageSync('rd_session'), formId: formId }, function (sendTemplate) {
    ...
    doSearch: function () {
        ...
        server.getJSON('https://xcx.so50.com/Pages/ajaxsqtg/GetProdatasqtindexuse.ashx', {userid: wx.getStorageSync('rd_session'),tit: self.data.searchWords,scene:self.data.scene ,showmodel: '', page: self.data.page, page_size: self.data.page_size }, function (res) {
             wx.setStorageSync('GetProdatasqtindexuse', res);
               wx.setStorageSync('StorageSecond', 0);
               self.setData({
    ...
    onLoad: function (options) {
        ...
        var res = wx.getStorageSync('GetProdatasqtindexuse');
        if (res) {
            self.setData({
        ...
        server.getJSON('https://xcx.so50.com/Pages/ajaxsqtg/GetProdatasqtindexuse.ashx', {userid: wx.getStorageSync('rd_session'),scene:scene,showmodel: '', page: self.data.page, page_size: self.data.page_size }, function (res) {
            wx.setStorageSync('GetProdatasqtindexuse', res);
            var StorageSecond = Date.parse(new Date())/1000+10;
            wx.setStorageSync('StorageSecond', StorageSecond);
  ...
  initread: function () {
   var self = this;
            server.getJSON('https://xcx.so50.com/Pages/ajaxsqtg/GetProdatasqtindexusedelayedPage.ashx', {userid: wx.getStorageSync('rd_session'),scene:self.data.scene ,showmodel: '', page:1 }, function (reslayedpay) {
                    ...
                    server.getJSON('https://xcx.so50.com/Pages/ajaxsqtg/GetProdatasqtindexusebar.ashx', { userid: wx.getStorageSync('rd_session'),scene:self.data.scene }, function (res) {
                        self.setData({
                            barProduct: res.data.barProduct
                        });
                    });
                    if (StorageSecond- Date.parse(new Date())/1000<0) {
