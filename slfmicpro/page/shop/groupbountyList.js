...
Page({
    ...
    onLoad: function (options) {
        var self = this;
        server.getJSON('https://xcx.so50.com/Pages/Ajax/Getbounty_listData.ashx', { bountyid: options.bountyid,bounty_infoid: options.bounty_infoid }, function (res) {
            self.setData({
    ...
    onShareAppMessage: function (res) {
        ...
        return {
            ...
            path: '/page/shop/groupbountyshare?bounty_infoid=' + self.data.bounty_infos[0].id,
    ...
    tapPddbounty_infosBuy: function (e) {
             var rd_session = wx.getStorageSync('rd_session');
        server.getJSON('https://xcx.so50.com/Pages/Ajaxwx/pddbountyBuy.ashx', { bounty_infoid: self.data.bounty_infos[0].id, userid: rd_session }, function (res) {
            if (res.data.results[0].errormess == '购买成功') {
                wx.showModal({
                    title: '提示',
                    content: res.data.results[0].errormess,
