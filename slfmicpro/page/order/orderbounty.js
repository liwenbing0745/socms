...
Page({
    ...
    onLoad: function (options) {
        ...
        server.getJSON('https://xcx.so50.com/Pages/Ajax/GetOrderbountyData.ashx', { userid: rd_session }, function (res) {
            self.setData({
    ...
    addCart: function (id) {
        var self = this;
        server.getJSON('https://xcx.so50.com/Pages/Ajax/UpdateSumorderbounty.ashx', { id: id, buy_sum: 1 }, function (res) {
            self.setData({
                confirmSubmit: res.data.confirmSubmit,
                goodsList: res.data.goodsList,
                goods: res.data.goods,
    ...
    submit: function (e) {
        ...
        var rd_session = wx.getStorageSync('rd_session');
        var value = e.detail.value;
        server.getJSON('https://xcx.so50.com/Pages/Ajaxwx/UserAddOrderbounty.ashx', { userid: rd_session, openid: openid, deliveraddids: value.id, remark: value.remark, invoiceH: value.invoiceH }, function (res) {
                ...
                wx.requestPayment({
                    ...
                    'success': function (res) {
                        server.getJSON('https://xcx.so50.com/Pages/Ajaxwx/UpdateOrderState.ashx', { id: res.data.results[0].id, sp_billno: res.data.results[0].order_no }, function (payres) {
                            wx.navigateTo({ url: '/page/shop/groupbountyList?bounty_infoid=' + res.data.results[0].bounty_infoid + '&bountyTyp=' + res.data.results[0].bountyTyp + '&bountyid=' + res.data.results[0].bountyid + '' });
                        });
//                        wx.showModal({
//                            title: '提示',
//                            content: "支付成功",
